...
int ModbusSlave::validate_request(unsigned char *data)
{
...
int ModbusSlave::write_holding_register(unsigned int write_addr, unsigned char *query, uint16_t *regs)
{
        ...
        //Evento cuando registro "holding Register" ha sido actualizado
        if(writeholdingregister != NULL)
...
int ModbusSlave::update()
{
	...
	exception = validate_request(query);
